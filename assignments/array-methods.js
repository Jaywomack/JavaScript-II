...
let ticketPriceTotal = runners.reduce( function(total, currentRunner) {
...
// Problem 1
//All of the employees from Skinix get special pink tshirts seperate them into an array called skinixEmployees
let skinixEmployees = [];

runners.filter( function(object) {
    let name = `${object.first_name} ${object.last_name}`
    if(object.company_name === 'Skinix') {
        skinixEmployees.push(name)
    }
...
// Problem 2
//Create an array of all the emails from the runners so that we can send them thousands of emails about our new foot lotion
let emailList = [];
runners.map( function(object) {
...
// Problem 3
//find the average donation amount so we can guilt runners for being to cheap
let averageDonation = runners.reduce( function(total, currentRunner) {
    return Math.floor((total + currentRunner.donation) / 50)
},0)
